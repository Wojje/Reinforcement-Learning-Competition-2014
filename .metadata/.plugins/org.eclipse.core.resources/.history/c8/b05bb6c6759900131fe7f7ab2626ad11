...
public class suttonandbarto44 {
	...
	private static double[][] poissons = {
	...
	private static double poisson(int expected, int n) {
		if (poissons[expected][n] != -1.0) {
		...
		return poissons[expected][n] = temp/nfac*Math.exp(-1*expected); 
	}
						...
						for (int i = 0; i < 15; i++) {
							...
							for (int j = 0; j < 15; j++) {
								...
								for (int k = 0; k < 15; k++) {
									//lot 2 requests
									for (int l = 0; l < 15; l++) {
										double probability = poisson(3, i) *
												poisson(3,j) * poisson(2,k) *
												poisson(4,l);
