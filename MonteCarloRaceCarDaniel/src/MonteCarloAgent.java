...
public class MonteCarloAgent implements AgentInterface {
    ...
    private int getBestAction(int[] state) {
    	...
    	for (int i = 1; i < 10; i++ ){
    		RewardCount rc = q.get(new StateAction(state, i));
    		if (rc != null) {
    	...
    	//softmax
    	if (randGenerator.nextDouble() > alpha) {
    		return randGenerator.nextInt(9) + 1;
    	}    	
    	return bestAction;    	
    }
    ...
    public void agent_end(double reward) {
    	...
    	for(StateAction sa : actionsTaken) {
    		int[] state = sa.state;
    		if (debug) System.out.println("State: " + state[0] + " " + state[1] + 
    				" " + state[2] + " " + state[3] + " action: " + sa.action);
    		
    		RewardCount rc = q.get(new StateAction(state, sa.action)); 
    		if (rc != null) {
    			rc.averageReward = (rc.averageReward * rc.count + totalReward) / 
    					((double) (rc.count + 1));
    			rc.count++;
    		} else {
    			q.put(sa, new RewardCount(1, totalReward));
    		}
    	}
    }
